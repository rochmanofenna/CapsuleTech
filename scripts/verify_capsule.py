...
def _verify_event_log_chain(path: Path, anchor_hash: str) -> bool:
    prev_hash = "0" * 64
...
def _verify_capsule_core(
    ...
    *,
    ...
    anchors_expected = []
    anchor_meta = capsule.get("anchor") or {}
    event_chain_head = anchor_meta.get("event_chain_head")
    ...
    if event_chain_head:
        ...
        events_hash_expected = anchor_meta.get("events_log_hash")
        if events_hash_expected:
            ...
            if actual_events_hash.lower() != str(events_hash_expected).lower():
                return E201_EVENT_LOG_MISMATCH, None
    if anchor_meta:
